...
int countBagTreev2(vector<bag> Bags, int childid) {
	int sum = 0;
	for (int i = 0; i < Bags.size(); i++) {
		for (int j = 0; j < Bags[i].child.size(); j++) {
			if (Bags[i].child[j].first == childid) {
				sum += 1 + countBagTreev2(Bags, i);
			}
		}
	}
	return sum;
...
int countBagTree(vector<bag> Bags, int parent_index) {
	int sum = 0;
	if (Bags[parent_index].parentId.size() == 0) {
		return 1;
	} else {
		for (int i = 0; i < Bags[parent_index].parentId.size(); i++) {
			sum += countBagTree(Bags, Bags[parent_index].parentId[i]);
		}
	}
	return sum;
}

bool containsBag(vector<bag> Bags, bag current_bag, int search_index) {
	bool contains = false;
	if (current_bag.child.size() == 0) {
		return false;
	} else {
		for (int i = 0; i < current_bag.child.size(); i++) {
			if (current_bag.child[i].first == search_index) return true;
			contains =
				contains || containsBag(Bags, Bags[current_bag.child[i].first],
										search_index);
		}
	}
	return contains;
...
int main() {
	...
	int sum = 0;
	for (int i = 0; i < Bags.size(); i++) {
		if (containsBag(Bags, Bags[i], Shiny)) sum++;
	}
